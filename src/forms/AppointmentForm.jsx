...
const schema = yup.object().shape({
  ...
  preferredHairdresser: yup
    .string()
    .required("Preferred staff is required"),
...
const AppointmentForm = ({ appointmentToEdit }) => {
  ...
  const { selectedSlot, selectedStaff } = location.state || {};
  ...
  const {
    ...
    { staffId: staff, page: 1, limit: 1000 },
    { skip: !staff }
  );
  ...
  useEffect(() => {
    if (selectedDay && staff) {
      refetchDayAppointments();
    }
  }, [selectedDay, staff, refetchDayAppointments]);
  ...
  useEffect(() => {
    if (appointmentToEdit) {
      ...
      setStaff(preferredHairdresser);
      setSelectedCategory(serviceCategory);
    } else if (selectedStaff || selectedSlot) {
      setValue("preferredHairdresser", selectedStaff);
      setValue("appointmentDateTime", selectedSlot);
    }
  }, [appointmentToEdit, selectedStaff, selectedSlot, setValue]);
