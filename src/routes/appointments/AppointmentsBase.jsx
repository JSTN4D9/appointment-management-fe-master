...
import {
  ...
  useFetchAppointmentsByStaffQuery,
...
const AppointmentsBase = () => {
  ...
  const { data: staffsData } = useFetchStaffsQuery();
  const staffs = useMemo(() => staffsData?.results || [], [staffsData]);

  const getStaffName = (staffId) => {
    const staff = staffs.find((b) => b.id === staffId);
    return staff ? `${staff.firstName} ${staff.lastName}` : "Loading...";
  };
  ...
  switch (user?.role) {
    case "admin":
      ...
      break;
    case "staff":
      appointmentsQuery = useFetchAppointmentsByStaffQuery({
        staffId: userId,
        page: paginationModel.page + 1,
        limit: paginationModel.pageSize,
      });
  ...
  // Fetch all appointments for the staff
  const { data: dayAppointments = { results: [] } } =
    useFetchAppointmentsByDayAndStaffQuery(
      { staffId: user?.id, page: 1, limit: 1000 },
      { skip: !user?.id }
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: "Staff",
      width: 150,
      renderCell: (params) => getStaffName(params.row.preferredHairdresser),
    },
    {
      field: "price",
      headerName: "Price",
